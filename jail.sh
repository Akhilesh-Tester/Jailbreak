...
echo "[*] Command ran:`if [ $EUID = 0 ]; then echo " sudo"; fi` ./jail.sh $@"
...
echo "jail | Version $version-$branch-$commit"
...
if [ "$(get_device_mode)" = "normal" ]; then
    ...
    if [ "$arch" = "arm64e" ]; then
        echo "[-] jail doesn't, and never will, work on non-checkm8 devices"
        exit
    fi
...
if [ "$(get_device_mode)" != "dfu" ]; then
    recovery_fix_auto_boot;
    _dfuhelper "$cpid" || {
        echo "[-] failed to enter DFU mode, run jail.sh again"
        exit -1
    }
fi
sleep 2
